...
class Turn:
    def __init__(self, count, reg_limit, limit, board, player, card_history):
        self.__count = count
        ...
        for token in tokenOptions:
            ...
            for row in range(1, (self.__board.height() + 1)):
                ...
                for col in range(1, (self.__board.width() + 1)):
                    ...
                    if (col + 3 <= self.__board.width() and row+3 <= self.__board.height()):
                        if (self.__board.elementToken(tokenType, row, col) == token and
            ...
            for row in range(1, (self.__board.height() + 1)):
                ...
                for col in range(1, (self.__board.width() + 1)):
                    ...
                    if (col - 3  >= 1 and row+3 <= self.__board.height()):
                        if (self.__board.elementToken(tokenType, row, col) == token and
                                self.__board.elementToken(tokenType, row + 1, col - 1) == token and
                                self.__board.elementToken(tokenType, row + 2, col - 2) == token and
                                self.__board.elementToken(tokenType, row + 3, col - 3) == token):
            ...
            if(won):
                print('='*45)
                print('WE HAVE A WINNER : ', self.__player.name(), 'got 4', token, 'in-line!')
                print('=' * 35)
                return won
        ...
        while self.__completed != True:
            ...
            if move is not None:
                ...
                if (move.type()=='regular'):
                    ...
                    is_valid = self.validate_insert(card, x_rng, y_rng, coord[0], coord[1], coord[2], coord[3])
                            ...
                            if(desired_id!= previous_card_id):
                                ...
                                if(desType == 1 or desType == 3 or desType == 5 or desType == 7):
                                    ...
                                    if (self.__board.element(sel_row1 + 1, sel_col1) == '___'
                                        ...
                                        # clear space
                                        picked_content = self.pickUpCard(found_card, sel_row1, sel_col1, sel_row2, sel_col2)
                                        # different position
                                        if (dest_row != x1  or dest_col != y1):
                                            new_card = Card()
                                            coord = self.initialize_move(new_card, move)
                                            is_valid = self.validate_insert(new_card, x_rng, y_rng, coord[0], coord[1], coord[2], coord[3])
                                            if (is_valid):
                                                self.__card_history.remove(found_card)
                                                break
                                            if (not is_valid):
                                                # put it back
                                                self.putHorizontalCardBack(picked_content[0], picked_content[1], sel_row1, sel_col1, sel_row2, sel_col2)
                                                break
                                        ...
                                        # same position different rotation
                                            ...
                                            if (move.placement() != desType):
                                                new_card = Card()
                                                coord = self.initialize_move(new_card, move)
                                                is_valid = self.validate_insert(new_card, x_rng, y_rng, coord[0], coord[1], coord[2],
                                                                                coord[3])
                                                if (is_valid):
                                                    self.__card_history.remove(found_card)
                                                    break
                                                if (not is_valid):
                                                    ...
                                                    self.putHorizontalCardBack(picked_content[0], picked_content[1],
                                                                               sel_row1, sel_col1, sel_row2, sel_col2)
                                                    break
                                    ...
                                    if self.__board.element(sel_row2 + 1, sel_col2) == '___': # nothing on top
                                        # clear space
                                        picked_content = self.pickUpCard(found_card, sel_row1, sel_col1, sel_row2, sel_col2)
                                        ...
                                        if (dest_row != x1 or dest_col != y1):
                                            ...
                                            if (not is_valid):
                                                # put it back
                                                self.putVerticalCardBack(picked_content[0], picked_content[1], sel_row1, sel_col1, sel_row2,
                                                                           sel_col2)
                                                break
                                        ...
                                        # same position different rotation
                                            ...
                                            if(move.placement() == desType):
                                                ...
                                                self.putVerticalCardBack(picked_content[0], picked_content[1], sel_row1,
                                                                         sel_col1, sel_row2,
                                                                         sel_col2)
                                                break
    ...
    def validate_insert(self, card, x_rng, y_rng, x1, y1, x2, y2):
        msg_separator = '='*36 +'\n'
        valid_move = False
        # Move Validity Check
